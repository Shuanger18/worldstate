(() => {
  ...
  const media = (video, label, className='') => {
    ...
    return src ? `<video class="gif-video ${className}" data-src="${esc(src)}"${video.poster?` poster="${esc(safeUrl(video.poster))}"`:''} autoplay loop muted playsinline preload="none" controlslist="nodownload noremoteplayback noplaybackrate" disablepictureinpicture disableremoteplayback draggable="false" tabindex="-1" aria-label="${esc(label)}"></video>` : placeholder(label);
  };
