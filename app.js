(() => {
  ...
  const cover = `<figure class="featured-showcase featured-film" id="featured-video"><div class="featured-media">${feature.src?`<video src="${esc(safeUrl(feature.src))}" poster="${esc(safeUrl(feature.poster||''))}" controls playsinline preload="none" controlslist="nodownload nofullscreen" disablepictureinpicture disableremoteplayback aria-label="${esc(feature.title||'WorldState overview video')}"></video><button type="button" class="featured-play" aria-label="Play WorldState overview video"><svg viewBox="0 0 48 48" aria-hidden="true"><path d="M17 10 38 24 17 38Z" fill="currentColor"/></svg></button>`:placeholder(feature.title||'WorldState in motion',true)}</div></figure>`;
